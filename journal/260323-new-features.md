...
The 15s/30s increments above 60s were too coarse. The new 10s steps give much finer control in the 1-5 minute range where
most intervals land. The 30s steps above 5min are kept since precision matters less at longer durations.
...
Previously, completed and current segments both appeared dark, making it hard to tell which segment was active at a
glance.
...
This allows on-the-fly adjustment without restarting. The timeline is modified in place — `addRep()` appends work (and
rest if applicable) segments, `removeRep()` pops them off the end. No progress is lost.
